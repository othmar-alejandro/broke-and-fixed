...
function SeamlessVideo({ src }) {
  ...
  return (
    <>
      <video ref={aRef} src={src} muted playsInline preload="none" className="absolute inset-0 w-full h-full object-cover" style={{ opacity: 0, transition: `opacity ${CROSSFADE_MS}ms ease` }} />
      <video ref={bRef} src={src} muted playsInline preload="none" className="absolute inset-0 w-full h-full object-cover" style={{ opacity: 0, transition: `opacity ${CROSSFADE_MS}ms ease` }} />
    </>
  )
}
  ...
  return (
    <section ref={container} className="relative min-h-[100dvh] flex items-center justify-center overflow-hidden bg-cream">
      ...
      >
        ...
        <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse 70% 65% at 50% 50%, rgba(0,0,0,0.18) 0%, rgba(0,0,0,0.06) 55%, rgba(0,0,0,0.0) 100%)' }} />
      </div>
      ...
      <div className="relative w-full max-w-4xl mx-auto px-4 md:px-8 flex flex-col items-center text-center">
        ...
        >
          {t('hero.headline1')}
          <br />
          <span className="text-sage-muted">{t('hero.headline2')}</span>
        </h1>
